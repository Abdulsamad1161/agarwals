...
/* //checkmembership plan expiry
$(document).ready(function () {
    $.ajax({
        type: 'POST',
        url: MdsConfig.baseURL + '/AjaxController/checkMembeshipPlan',
        data: setAjaxData({})
    });
});

//check business membership plan expiry
$(document).ready(function () {
    $.ajax({
        type: 'POST',
        url: MdsConfig.baseURL + '/AjaxController/checkBusinessMembeshipPlan',
        data: setAjaxData({})
    });
}); */
...
$(document).ready(function () {
    $("#form_register").submit(function (event) {
        ...
        if (form[0].checkValidity() === false) {
            ...
            $.ajax({
                type: 'POST',
                url: MdsConfig.baseURL + '/register-post-new',
                data: serializedData,
                success: function (response) {
                    ...
                    if (obj.result == 1) {
						swal({
						  ...
						  text: "Thank you for submitting your information. You will receive all communications via the email you provided. Please proceed with booking your ticket.",
						  icon: "info",
						 confirmButtonText: 'ok',
						  dangerMode: true,
						}).then(function(isConfirm) {
						  if (isConfirm) {
							  window.location.href = MdsConfig.baseURL + '/';
						  }
						});
                    } 
						...
						swal({
						  title: "Sorry!",
						  text: "You cannot log in as a guest. Email is already registered. Password is required for login.",
						  icon: "warning",
						 confirmButtonText: 'ok',
						  dangerMode: true,
						}).then(function(isConfirm) {
...
//register
$(document).ready(function () {
    $("#form_register_2").submit(function (event) {
		
		var submitform = false;
			
		var captchaImage = $('#captchaImageForm').data('value');
		var captchatext = $('#captcha_text_form').val();
		if(captchaImage == 1)
		{
			var value1text = "D5GKX";
			if(value1text === captchatext)
			{
				submitform =  true;
			}
		}
		else if(captchaImage == 2)
		{
			var value1text = "b42yb";
			if(value1text === captchatext)
			{
				submitform =  true;
			}
		}
		else if(captchaImage == 3)
		{
			var value1text = "upord";
			if(value1text === captchatext)
			{
				submitform =  true;
			}
				...
				$.ajax({
					...
					success: function (response) {
						...
						if (obj.result == 1) {
							swal({
							  title: "Submitted Successfully!",
							  text: "Thank you for submitting your information. You will receive all communications via the email you provided.",
							  icon: "info",
							 confirmButtonText: 'ok',
							  dangerMode: true,
							}).then(function(isConfirm) {
		...
		else
		{
			swal({
						...
						text: "You're Not allowed to submit",
					});
			return false;
		}
    });
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (obj.result == 1) {
						if(obj.otp == 1)
						{
							window.location.href = MdsConfig.baseURL + '/AuthController/otpVerification';
						}
						else
						{
							location.reload();
...
//Otp-login
$(document).ready(function () {
    $("#form_login_otp").submit(function (event) {
        var form = $(this);
        if (form[0].checkValidity() === false) {
            event.preventDefault();
            event.stopPropagation();
            ...
            $.ajax({
                type: 'POST',
                url: MdsConfig.baseURL + '/AuthController/otpVerificationPost',
                data: serializedData,
                success: function (response) {
                    var obj = JSON.parse(response);
					document.getElementById("result-login-otp").innerHTML = obj.errorMessage;
                    if (obj.result == 1) {
                        window.location.href = MdsConfig.baseURL +  '/AuthController/redirectSuccessfulOTP';
