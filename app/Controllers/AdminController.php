...
class AdminController extends BaseAdminController
{
    ...
    */
	...
	{ 
		...
		$data['registrationFormList'] = $this->settingsModel->getRegistrationFormData();
	...
	public function eventRegistrationFormEdit($id)  /*  Custom Added */
	{ 
		$data["title"] = 'Edit Registration Form';
		$data["eventsList"] = $this->settingsModel->getAllEventsList();
		$data["galleryVideos"] = $this->settingsModel->getEditEventRegistrationForm($id);

		echo view('admin/includes/_header', $data);
        echo view('admin/event_management/editRegistrationForm',$data);
        echo view('admin/includes/_footer');
	}
	...
	public function registrationFormPost()
	{
		if ($this->settingsModel->registrationFormPost()) 
		{
            setSuccessMessage(trans("msg_updated"));
        } 
		else 
    ...
    */
	...
	public function boardOfDirectorsSub()
	{
		$data['title'] = trans("board-of-directors");
        $data['boardMembers'] = $this->pageModel->getboardMembersSub();

        $data['panelSettings'] = getPanelSettings();
        echo view('admin/includes/_header', $data);
        echo view('admin/board_directors/board_directors_index_sub', $data);
        echo view('admin/includes/_footer');
	}
	...
	public function addboardOfDirectorsSub()
    {
		$data = [];
        $data['title'] = trans("add_board_directors");
		$data['members'] = $this->pageModel->getAllMembersList();
		$order = $this->pageModel->getOrderOfDirectorsSub();
		$incrementedOrder = $order + 1;

		$data['order'] = $incrementedOrder; 
	
        echo view('admin/includes/_header', $data);
        echo view('admin/board_directors/add_board_directors_sub', $data);
        echo view('admin/includes/_footer');
    }
	...
	public function editboardOfDirectorsSub($id)
    {
        $data['title'] = trans("edit_director");
 	    $data['boardMembers'] = $this->pageModel->getboardMembersEditSub($id);
		$data['members'] = $this->pageModel->getAllMembersList();
		
        echo view('admin/includes/_header', $data);
        echo view('admin/board_directors/edit_board_directors_sub', $data);
        echo view('admin/includes/_footer');
    }
	...
	public function addBoardDirectorsSubPost()
    {
		if ($this->pageModel->addBoardDirectorsSub()) 
		{
			setSuccessMessage(trans("msg_added"));
			return redirect()->back();
		}
	...
	public function editBoardDirectorsSubPost()
    {
		if ($this->pageModel->editBoardDirectorsSub()) 
		{
			setSuccessMessage(trans("msg_added"));
			return redirect()->back();
		}
	...
	public function editRegistrationFormPost()
    {
		if ($this->settingsModel->editRegistrationFormPost()) 
		{
			setSuccessMessage(trans("msg_updated"));
			return redirect()->back();
		}
	...
	public function deleteBoardDirectorImageSubPost()
    {
        $id = inputPost('id');
       
		if ($this->pageModel->deleteBoardDirectorImageSub($id)) 
		{
			setSuccessMessage(trans("msg_deleted"));
		} 
		else 
	...
	public function deleteBoardDirectorSubPost()
    {
        $id = inputPost('id');
       
		if ($this->pageModel->deleteBoardDirectorSub($id)) 
		{
			setSuccessMessage(trans("msg_deleted"));
		} 
		else 
	...
	public function deleteRegistrattionFormDataPost()
    {
        $id = inputPost('id');
       
		if ($this->settingsModel->deleteRegistrattionFormDataPost($id)) 
		{
			setSuccessMessage(trans("msg_deleted"));
		} 
		else 
	...
	private function uploadFile($file, $directory, $namePrefix, $keepOrjName, $maxSizeMB)
	{
		if (!empty($file) && !empty($file->getName())) {
			$orjName = $file->getName();
			$ext = pathinfo($orjName, PATHINFO_EXTENSION);
			...
			setErrorMessage('File should be less than 100mb');
		}
	...
	public function documentsList()
    {
        ...
        echo view('admin/includes/_header', $data);
        echo view('admin/documents/index', $data);
        echo view('admin/includes/_footer'); 
    }
	
	public function addDocumentsPost()
	{
		$directory = 'uploads/pdf-files/';
		$namePrefix = 'Document_'; 
		$keepOrjName = false;
		...
		if ($file_data) 
		{
			 $insertData = [
				...
				'document' => $file_data['path'],
				'order' => inputPost('order'),
				'uploaded_by' => user()->id
            ];
			
            if ($this->settingsModel->addDocumentsPost($insertData)) { 
                setSuccessMessage(trans("msg_added"));
            } else {
                setErrorMessage(trans("msg_file_upload_error"));
            }
	...
	{ 
		...
		$data["documentData"] = $this->settingsModel->editDocument($id);

		echo view('admin/includes/_header', $data);
		echo view('admin/documents/edit', $data);
        echo view('admin/includes/_footer');
	}
	
	public function editDocumentsPost() 
	{ 
		$id = inputPost('id');
		$directory = 'uploads/pdf-files/';
		$namePrefix = 'Document_'; 
		...
		if ($file_data) 
		{
            ...
            if ($this->settingsModel->editDocumentsPost($insertData,$id)) { 
                setSuccessMessage(trans("msg_updated"));
            } else {
                setErrorMessage(trans("msg_file_upload_error"));
            }
	...
	public function deleteDocumentsPost()
    {
        $id = inputPost('id');
       
		if ($this->settingsModel->deleteDocumentsPost($id)) 
		{
			setSuccessMessage(trans("msg_deleted"));
		} 
		else 
