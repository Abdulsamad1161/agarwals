...
class ProfileController extends BaseController
{
    ...
    public function rsvpEventForm_jot()
    {
        /* if (!authCheck()) {
            return redirect()->to(langBaseUrl());
        } */
        ...
        $data['keywords'] = trans("rsvpEventForm") . ',' . $this->baseVars->appName;
        $data['userSession'] = getUserSession();
        $data['form_data'] = $this->rsvpFormModel->getAllFormsRSVPJOT();
        echo view('partials/_header', $data);
        echo view('members/rsvp_form_jot', $data);
        echo view('partials/_footer');
    }
    ...
    public function rsvpEventFormPost()
    {
        ...
        if ($id = $this->rsvpFormModel->addRSVPformData()) {
			$formData = $this->rsvpFormModel->getSubmitFormData($id);
			$email = $this->find_email_in_object($formData);
			...
			$dataLabels = $this->rsvpFormModel->getLabelsFormUploaded($formID,$id);
			$formName = $this->rsvpFormModel->getFormName($formID);
					// Initialize content array
			$content = [];

			// Loop through the data fetched
			foreach ($dataLabels as $index => $label) {
				// Construct content based on field type
				switch ($label->fieldType) {
					case 'text':
						// For text fields, simply append the label and value
						$content[$index + 1] = $label->formLabel . ': ' . $label->value;
						break;
					case 'number':
						// For number fields, calculate content based on quantity (if available)
						if (isset($label->qty)) {
							$subtotal = number_format((float)($label->value * $label->qty), 2);
							$content[$index + 1] = $label->formLabel . ': ' . $label->value.' * '.$label->qty.' = '. $subtotal;
						} else {
							$content[$index + 1] = $label->formLabel . ': ' . $label->value;
						}
						break;
			...
			$emailContent = [
				'content' => $formName->form_name
			];

			foreach ($content as $index => $value) {
				if ($index < 20) {
					$emailContent['content_' . ($index + 1)] = $value;
				}
			}
			...
			$emailData = [
				'email_type' => 'activation',
				'email_address' => $email,
				'email_data' => serialize($emailContent),
				'email_priority' => 1,
				'email_subject' => 'RSVP Confirmation',
				'template_path' => 'email/rsvp_payment'
			];
			//echo "EMAIL DATA : <pre>";print_r($emailData);
			addToEmailQueue($emailData);
    ...
    public function paymentUsers()
    {
		...
		if ($paymentType == 'rsvpPayment') {
            ...
            $data['rsvpForm'] = $this->rsvpFormModel->getFormData($formId);
			$rsvpFormDetails = $data['rsvpForm']->form_name;
			helperSetSession('rsvpFormDetails',$rsvpFormDetails);
            if (empty($data['rsvpForm'])) {
                return redirect()->to(langBaseUrl());
            }
            //total amount
	...
	public function rsvpPaymentCompleted()
    {
		...
		$formData = $this->rsvpFormModel->getSubmitFormData($submit_id);
		$email = $this->find_email_in_object($formData);
		...
		$dataLabels = $this->rsvpFormModel->getLabelsFormUploaded($formID,$submit_id);
		$formName = $this->rsvpFormModel->getFormName($formID);
				// Initialize content array
		$content = [];
		...
		// Add total amount as content_20
		if (isset($dataLabels[0]->total)) {
			$emailContent['content_20'] = 'TOTAL Amount - ' . $dataLabels[0]->total;
		}
	...
	public function find_email_in_object($data) {
	...
	public function rsvp_payment_epyament()
	{
		...
		//$emailid = $this->authModel->getUser(user()->id);
