...
class TicketBookingController extends BaseController
{
	...
	public function generatePdf()
	{
		...
		$data_id = inputPost('data_id');
		$data_mem_id = inputPost('data_mem_id');
		$data['tickets'] = $this->ticketModel->getBookedTicketDataTrans($data_id,$data_mem_id);
        ...
        $pdfPath = WRITEPATH . 'uploads/invoice/'.$data['tickets'][0]['transaction_id'].'.pdf'; // Adjust the path as needed
		...
		if ($result === false) {
			die('Failed to write PDF file to ' . $pdfPath);
		}

		// Check if the file was created successfully
		if (!file_exists($pdfPath)) {
			die('PDF file not created at ' . $pdfPath);
		}

		// Output successful message or handle accordingly
		echo 'PDF file created successfully at ' . $pdfPath;
	...
	public function generatePdf_test()
	{
		echo date('Y-m-d H:i:s');
		$data_id = $_GET['trans_id'];
		$data_mem_id = $_GET['id_mem'];
		$data['tickets'] = $this->ticketModel->getBookedTicketDataTrans($data_id,$data_mem_id);
		//echo "<pre>";print_r($data['tickets']);die;
		$options = new Options();
        $options->set('isHtml5ParserEnabled', true);
	...
	public function getTableNamesFromDefine()
	{
		// Retrieve the JSON data from the POST request
		$request = \Config\Services::request();
		$values = $request->getJSON(true)['values'];

		// Check if values are provided
		if (empty($values) || !is_array($values)) {
			return $this->response->setJSON(['error' => 'No values provided']);
		}
		...
		try {
			...
			foreach ($values as $value) {
				// Query the database for each value
				$data = $this->ticketModel->getTableNameDefineTable($value);
				// Store the result with the table name as the key
			...
			return $this->response->setJSON(['error' => 'An error occurred']);
		}
	}
