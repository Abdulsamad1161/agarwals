...
class MembershipPlansUsersController extends BaseController
{
	...
	public function showPlans()
    {
		...
		if(!empty($userPlan))
		{
			if(!empty($data['checkPlanExists']))
			{
				...
				elseif($daysLeft <= 30)
				{
					setErrorMessage("Heads up! Your membership will expire in $daysLeft day" . ($daysLeft !== 1 ? 's' : '') . ". Renew now to continue enjoying your benefits!");
				}
				elseif($daysLeft < 0)
				{
	...
	public function membershipPlanChoose($planId)
    {
		if (isMember()) {
			...
			$expirationDateString = $userPlan->plan_end_date; 
			...
			if ($expirationDate <= $currentDate || $daysUntilExpiration <= 30) {
    ...
    public function paymentUsers()
    {
		...
		if ($paymentType == 'membership') {
			...
			$userdata = $this->authModel->getUser(user()->id);
		
            $data['planTitle'] = $data['plan']->title.' '.$data['plan']->number_of_days.' Name : '.$userdata->username.' Email : '.$userdata->email.' Phone : '.$userdata->phone_number;
			helperSetSession('plan_purchase_details', $data['planTitle']);
