...
class MembershipController extends BaseAdminController
{
    ...
    public function users()
    {
        ...
        $data['users'] = $this->membershipModel->getPaginatedUsers();
	...
	public function deleteSelectedUsers(){
		$userIds = $this->request->getVar('user_ids');
		
		if (!empty($userIds)) {
            $userIdsArray = explode(',', $userIds);
            foreach ($userIdsArray as $id) {
                $status = $this->membershipModel->deleteUser($id);
				if(!$status){
					break;
				}
            }
		...
		setErrorMessage('No users found to delete');
		return redirect()->back();
	}
	...
	public function SetSelectedUsers()
	{
		...
		if(!empty($users))
		{
			foreach($users as $user)
			{
				...
				$success = $this->authModel->registerMigrationUpdate($user->id, $content, $pass);
			...
			if (empty($errorMessages)) {
				setSuccessMessage('Email Has Been Sent To The Unconfirmed Members');
			}
			else 
			{
				$errorMsg = implode("<br>", $errorMessages);
	...
	public function updatePlanUsers($id)
    {
        $data['title'] = 'Add Membership Plan';
        $data['user'] = getUser($id);
        if (empty($data['user'])) {
            return redirect()->to(adminUrl('members'));
        }
        ...
        $data['panelSettings'] = getPanelSettings();
        echo view('admin/includes/_header', $data);
        echo view('admin/membership/updatePlan', $data);
        echo view('admin/includes/_footer');
    }
	
	public function addMembershipPlan()
	{
		if ($this->membershipModel->addMembershipPlanManually()) {
			$email = inputPost('email');
			$user = $this->authModel->getUserByEmail($email);
			$plan = $this->membershipModel->getPlansUsersManually($user->id);
			$emailData = 
			[
					...
					'email_data' => serialize([
						...
						'content_1' => 'Plan Name : '.$plan->plan_title,
						'content_2' => 'Plan Expiry : '.formatDateShort($plan->plan_end_date),
						'content_3' => 'Payment Status : '.$plan->payment_status,
						'content_4' => "Thank you! If you have any questions or need assistance, feel free to reach out.",
					]),
					'email_priority' => 1,
			...
			addToEmailQueue($emailData);
            setSuccessMessage('Membership Plan Added successfully.');
        } else {
            setErrorMessage('Error Occured Adding the Plan');
        }
        redirectToBackUrl();
	}
	
	public function SetSelectedUsersEmail(){
		//echo "<pre>";print_r($_POST);
		
		$content = inputPost('content');
        $content = nl2br($content);
		$users_id = inputPost('users_id');
		...
		if(!empty($users_ids)){
			foreach($users_ids as $user_id){
				...
				$success = $this->authModel->registerMigrationUpdate($user->id, $content);
