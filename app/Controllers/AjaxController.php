...
class AjaxController extends BaseController
{
	...
	public function checkMembeshipPlan()
	{
		if(!empty(authCheck()))
		{
		...
		if(!empty($userPlan))
		{
			...
			if ($daysLeft === 30) {
				...
				$this->sendExpirationAlert($userPlan, 30);
			} elseif ($daysLeft === 5) {
				// Send an alert 5 days before expiration
				$this->sendExpirationAlert($userPlan, 5);
			} elseif ($daysLeft === 1 || $daysLeft === 0) {
				// Send alerts on the last day and the day of expiration
				$this->sendExpirationAlert($userPlan, 1);
				$this->sendExpirationAlert($userPlan, 0);
			}
			elseif($daysLeft < 0)
			{
	...
	private function sendExpirationAlert($userPlan, $daysBeforeExpiration)
	{
		$messageShownToday = $this->checkIfMessageShownTodayForPlan($userPlan->id);
		...
		if (!$messageShownToday) {
			...
			$this->markMessageAsShownTodayForPlan($userPlan->id);
		}
	}

	
	function checkIfMessageShownTodayForPlan($id) 
	{
		$data = $this->membershipModel->msgUserSentExpiry($id);
		if ($data) {
	...
	function markMessageAsShownTodayForPlan($id) {
		return $this->membershipModel->msgUserSentExpiryToday($id);
	}
	
	public function checkBusinessMembeshipPlan()
	{
		if(!empty(authCheck()))
		{
		...
		if(!empty($userPlan))
		{
			...
			if ($daysLeft === 30) {
				// Send an alert 30 days before expiration
				$this->sendExpirationAlertbm($userPlan, 30);
			} elseif ($daysLeft === 5) {
				// Send an alert 5 days before expiration
				$this->sendExpirationAlertbm($userPlan, 5);
			} elseif ($daysLeft === 1 || $daysLeft === 0) {
				// Send alerts on the last day and the day of expiration
				$this->sendExpirationAlertbm($userPlan, 1);
				$this->sendExpirationAlertbm($userPlan, 0);
			}
		}
	}
	
	private function sendExpirationAlertbm($userPlan, $daysBeforeExpiration)
	{
		$messageShownToday = $this->checkIfMessageShownTodayForPlanbm($userPlan->id);
		if (!$messageShownToday) {
			$emailData = [
				'email_type' => 'activation',
				'email_address' => user()->email,
			...
			addToEmailQueue($emailData);
			// Set the flag indicating that the message has been shown today
			$this->markMessageAsShownTodayForPlanbm($userPlan->id);
		}
	}
	
	function checkIfMessageShownTodayForPlanbm($id) 
	{
		$data = $this->membershipModel->msgUserSentExpirybm($id);
		if ($data) {
			// Get the date when the message was last sent
			$msgSentDate = formatDate($data->msgSentDate);
	...
	// Function to mark the message as shown today for a specific plan
	function markMessageAsShownTodayForPlanbm($id) {
		return $this->membershipModel->msgUserSentExpiryTodaybm($id);
	}
}
