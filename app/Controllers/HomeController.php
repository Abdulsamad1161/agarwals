...
class HomeController extends BaseController
{
    ...
    public function index()
    {
		...
		$data['event_images'] = $this->settingsModel->getAllEventImages();
		$data['homePageImages'] = $this->settingsModel->getAllhomePageImages();
		$data['ourSponsors'] = $this->membershipModel->getAllOurSponsors();
		$charity = array();
		$tickets = array();
		$charity = $this->ticketModel->getCharityListVisible();
		$tickets = $this->ticketModel->getTickets();
		$data['tickets'] = array_merge($charity,$tickets);
     ...
     public function termsConditions()
    {
        ...
        $datas['page'] = $this->pageModel->getPageContentTermsCondition();
        if (empty($page)) {
            return redirect()->to(langBaseUrl());
        }
        if ($page->visibility == 0) {
            ...
            echo view('partials/_header', $data);
            //echo view('page', $data);
			echo view('terms_conditions',$datas);
            echo view('partials/_footer');
        }
    }
	...
	/**
     * Invoice Membership Users Plans
     */
    public function invoiceMembershipUsers($id)
    {
        $data['title'] = trans("invoice");
        $data['description'] = trans("invoice") . ' - ' . $this->baseVars->appName;
        $data['keywords'] = trans("invoice") . ',' . $this->baseVars->appName;
        ...
        $membershipModel = new MembershipModel();
        $data['transaction'] = $membershipModel->getMembershipTransactionUsers($id);
        if (empty($data['transaction'])) {
            return redirect()->to(langBaseUrl());
        }
        if (!hasPermission('membership')) {
        ...
        echo view('invoice/invoice_membership_users', $data);
    }
