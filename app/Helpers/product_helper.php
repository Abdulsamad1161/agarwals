    ...
    function getProductFormData($product)
    {
        ...
        if (!empty($product)) {
            ...
            if ($product->listing_type == 'sell_on_site' || $product->listing_type == 'license_key') {
                ...
                if (authCheck()) {
                    $data->button = '<button type="button" onclick="buisnessCustomerCommunication(' . $product->id . ', ' . $product->category_id . ', ' . $product->user_id . ')" class="btn btn-md btn-block btn-product-cart" data-toggle="modal" data-target="#messageModal">' . trans("contact_seller") . '</button>';
                } else {
