...
class MembershipModel extends BaseModel
{
    ...
    public function addMembershipTransaction($dataTransaction, $plan)
    {
		...
		if (date('m', strtotime($planStartDate)) >= 1 && date('m', strtotime($planStartDate)) <= 10) {
			// If purchased between January and October, expiry date is December of the same year
			$planEndDate = date('Y-12-31', strtotime($planStartDate));
		} elseif (date('m', strtotime($planStartDate)) >= 11) {
			// If purchased in November or December, expiry date is December of the next year (with a grace month)
			$planEndDate = date('Y-12-31', strtotime($planStartDate . ' + 1 year'));
		}
    ...
    public function getUserPlanByUserIdUsers($userId, $onlyActive = true)
    {
        ...
        return $this->builderUsersMembershipPlansSubs->where('deleted',0)->orderBy('id','desc')->limit('1')->where('user_id', clrNum($userId))->get()->getRow();
    }
	...
	public function msgUserSentExpiry($id)
	{
		return $this->builderUsersMembershipPlansSubs->where('id',$id)->where('user_id',user()->id)->where('deleted',0)->where('plan_status',1)->get()->getRow();
	}
	
	public function msgUserSentExpirybm($id)
	{
		return $this->builderUsersMembershipPlans->where('id',$id)->where('user_id',user()->id)->where('plan_status',1)->get()->getRow();
	}
	
	public function msgUserSentExpiryToday($id)
	{
		$today_date = formatDate(date('Y-m-d'));
		$data = array('msgSentDate' => $today_date);
		return $this->builderUsersMembershipPlansSubs->where('id',$id)->where('user_id',user()->id)->where('deleted',0)->where('plan_status',1)->update($data);
	}
	
	public function msgUserSentExpiryTodaybm($id)
	{
		$today_date = formatDate(date('Y-m-d'));
		$data = array('msgSentDate' => $today_date);
		return $this->builderUsersMembershipPlans->where('id',$id)->where('user_id',user()->id)->where('plan_status',1)->update($data);
	}
	...
	public function updateEpaymentApproveMembershipUsers()
	{
		...
		$userPlan = $this->getPlanUsers(inputPost('plan_id'));
		$planStartDate = date('Y-m-d');
