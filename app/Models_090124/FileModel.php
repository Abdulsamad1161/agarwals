...
class FileModel extends BaseModel
{
    ...
    public function __construct()
    {
        ...
        $this->builderGallery = $this->db->table('gallery_video');
    ...
    public function uploadGalleryVideo()
    {
        ...
        if ($this->storageSettings->storage == 'aws_s3') {
            $awsModel = new AwsModel();
            $storage = 'aws_s3';
            $file = $this->createFileName('videoGallery_');
            if (!empty($file['name']) && !empty($file['tempName'])) {
                $fileName = $file['name'];
                $awsModel->putVideoGalleryObject($file['name'], $file['tempName']);
            }
        } else {
            $file = $this->uploadModel->uploadVideo('file');
            if (!empty($file) && !empty($file['name'])) {
        ...
        if (!empty($fileName)) {
            ...
            $this->db->reconnect();
            $this->builderGallery->insert($data);
        }
    }
	...
	//get gallery video
    public function getProductVideoGallery($productId)
    {
        return $this->builderGallery->where('id', clrNum($productId))->where('media_type', 'video')->get()->getRow();
    }
    ...
    public function deleteGalleryVideo($productId)
    {
        $video = $this->getProductVideoGallery($productId);
        if (!empty($video)) {
            if ($video->storage == 'aws_s3') {
                $awsModel = new AwsModel();
                $awsModel->deleteVideoObject($video->file_name);
            ...
            return $this->builderGallery->where('id', $video->id)->delete();
        }
    }
