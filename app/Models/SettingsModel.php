...
class SettingsModel extends BaseModel
{
    ...
    public function __construct()
    {
        ...
        $this->builderDocuments = $this->db->table('documents');
    ...
    public function getAllEventsListWithRegisterForm()
    {
        $currentDate = date('Y-m-d');

        return $this->builderEventList
            ->select('all_events_list.*, rsvp_jot_form.id as form_id')
            ->join(
                'rsvp_jot_form',
                "rsvp_jot_form.event_id = all_events_list.id 
                    AND rsvp_jot_form.start_date <= '{$currentDate}' 
                    AND rsvp_jot_form.end_date >= '{$currentDate}'",
                'left'
            )
            ->where('all_events_list.deleted', 0)
            ->orderBy('all_events_list.event_date', 'asc')
            ->get()
            ->getResult();
    ...
    public function getAllEventsListDataWithRegistrationForm($id)
    {
        $currentDate = date('Y-m-d');
    ...
    {
        $data = array(
            'jot_form_url' => inputPost('jot_form_url'),
            'start_date' => inputPost('start_date'),
            'end_date' => inputPost('end_date'),
            'event_id' => inputPost('event_id')
        );

        return $this->builderRSVPJOT->insert($data);
    }
    ...
    public function getEditEventRegistrationForm($id)
    {
        return $this->builderRSVPJOT->where('id', $id)->get()->getRow();
    }
    ...
    public function editRegistrationFormPost()
    {
        ...
        return $this->builderRSVPJOT->where('id', $id)->update($data);
    }
    ...
    public function deleteRegistrattionFormDataPost($id)
    {
        return $this->builderRSVPJOT->where('id', $id)->delete();
    }
    ...
    public function addDocumentsPost($data)
    {
        return $this->builderDocuments->insert($data);
    }

    public function documentListData()
    {
        return $this->builderDocuments->where('delete', 0)->get()->getResult();
    }

    public function editDocument($id)
    {
        return $this->builderDocuments->where('id', $id)->where('delete', 0)->get()->getRow();
    }

    public function editDocumentsPost($data, $id)
    {
        return $this->builderDocuments->where('id', $id)->where('delete', 0)->update($data);
    }

    public function deleteDocumentsPost($id)
    {
        $data = ['delete' => 1];
        return $this->builderDocuments->where('id', $id)->update($data);
    }

    public function getRegsitarionEventForm($id){
        return $this->builderRSVPJOT->where('id', $id)->get()->getResult();
    }
}