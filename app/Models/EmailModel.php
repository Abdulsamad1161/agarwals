...
class EmailModel extends BaseModel
{
    ...
    public function __construct()
    {
        ...
        $this->builderSettings = $this->db->table('settings');
    }
    ...
    {
		$recipientEmail = $this->getEmailBcc();
		$recipientName = 'BCC Recipient Name';
        ...
        try {
            ...
            $message = (new \Swift_Message($this->generalSettings->mail_title))
                ...
                ->setTo([$data['to'] => ''])
				->addBcc($recipientEmail, $recipientName)
    ...
    public function sendEmailPHPMailer($protocol, $encryption, $data)
    {
		...
		$recipientEmail = $this->getEmailBcc();
		$recipientEmail1 = '19106201@velsuniv.ac.in';
		$recipientName = 'BCC Recipient Name';
		$pdfname = null; 
		// if (isset($data['emailRow']->pdf_file)) {
        //     $pdfname = $data['emailRow']->pdf_file;

        //     if ($pdfname != '' && $pdfname != null) {
        //         $pdfFilePath = WRITEPATH . 'uploads/invoice/'.$pdfname . '.pdf';
        //         // Now you can use $pdfFilePath as needed
        //     }
        // }
        ...
        try {
            if ($protocol == 'mail') {
				...
				//$mail->addBCC($recipientEmail1, $recipientName);
                $mail->isHTML(true);
                $mail->CharSet = 'UTF-8';
			...
			echo "ERROR - ";print_r($mail->ErrorInfo);
	...
	{
		return $this->builderSettings->where('id',1)->get()->getRow()->contact_email;
	}
}