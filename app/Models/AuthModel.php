...
class AuthModel extends BaseModel
{
    ...
    public function loginUser($user)
    {
        if (!empty($user)) {
            $userData = [
                'mds_ses_id' => $user->id,
				'mds_ses_username' => $user->username,
	...
	public function register()
    {
		...
		// Log registration attempt to a custom file
		$this->logRegistrationAttempt($data);
	...
	private function logRegistrationAttempt($data)
	{
		...
		// Get system name
		$systemName = php_uname('s') . ' ' . php_uname('r'); // Example: "Linux 5.4.0-42-generic"
		
		// Prepare log data as an associative array
		$logData = [
	...
	private function writeToLogFile($logData)
	{
		// Log file path
		$logFilePath = WRITEPATH . 'logs/registration_log.txt';
		
		// Create a log string (you can format it however you like)
		$logMessage = json_encode($logData) . PHP_EOL;  // Each log entry on a new line
		
		// Write the log message to the file
		if (file_put_contents($logFilePath, $logMessage, FILE_APPEND | LOCK_EX) === false) {
			// If writing to the file fails, log an error in the system log
			log_message('error', 'Failed to write to registration log file: ' . json_encode($logData));
		}
	}
    ...
    public function registerMigrationUpdate($mem_id,  $content, $pass=null)
    {
        if(!empty($pass)){
        ...
        if ($this->builder->update($data)) {
            ...
            if (!empty($user)) {
                if ($this->generalSettings->email_verification == 1) {
                    if(!empty($pass)){
                        $this->addActivationEmailMigrationUsers($user, $content, $pass);
                    } else {
                        $this->addActivationEmailMigrationUsers($user, $content);
                    }
                } 
            }
            return $id;
    ...
    public function logout()
    {
        ...
        $this->session->remove('guest_login');
    }
    ...
    {
        $this->buildQueryUser();
		$this->builder->where('users.email_status', 1);
	...
	//add activation email migration
    ...
    public function addActivationEmailMigrationUsers($user, $content, $pass=null, $email = null)
    {
        if ($this->generalSettings->email_verification == 1 && !empty($user)) {
            if (empty($email)) {
                $email = $user->email;
            ...
            if(!empty($pass)){
                $emailData = [
                    ...
                    'email_data' => serialize([
                        ...
                        'content_2' => 'Login Email : ' .$user->email,
                        'content_3' => 'Login Password : ' .$pass,
                        'content_4' => 'To begin, please log in first on our website: www.agarwals.ca under member login tab using the credentials provided above and then go to the "Profile Management" menu. From there, you can change your password to your own. Once done, please go to the Membership tab to Purchase/Renew your membership.',
                        'url' => base_url() . '/confirm-account?token=' . $token,
                        'buttonText' => trans("confirm_your_account")
                    ]),
                    'email_priority' => 1,
                    'email_subject' => 'Renew Your Membership for Continued Community Engagement',
                    'template_path' => 'email/migration'
                ];
            } else {
                $emailData = [
                    ...
                    'email_data' => serialize([
                        'content' => $content,
                        'content_1' => 'Your ABC Member ID : ' . 'ABC-' . $user->id,
                        'content_2' => 'Kindly log in to our website at www.agarwals.ca. After logging in, please proceed to the "Membership" page to complete your renewal.'
                    ]),
                    'email_priority' => 1,
                    'email_subject' => 'Renew Your Membership for Continued Community Engagement',
                    'template_path' => 'email/migration'
