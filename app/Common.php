...
//get username member
if (!function_exists('getUsernameMember')) {
    function getUsernameMember($user)
    {
        if (!empty($user)) {  
			return $user->first_name . ' ' . $user->last_name;
        }
...
if (!function_exists('isMember')) {
    function isMember()
    {
		$models = new \App\Models\ProfileModel();
        if ($models->getActiveMemberPlanDetails()) {
            return true;
        }
        return false;
    }
...
//is member abc with plan
if (!function_exists('isMemberPlan')) {
    function isMemberPlan()
    {
		$models = new \App\Models\ProfileModel();
        if ($models->getActiveMemberPlanDetailsAtleast()) {
            return true;
        }
        return false;
    }
...
if (!function_exists('isGuest')) {
    function isGuest()
    {
		$value = helperGetSession('guest_login');
		if($value === 1)
		{
			return true;
		}
