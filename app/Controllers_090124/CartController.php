...
class CartController extends BaseController
{
    ...
    public function membershipPaymentCompleted()
    {
		...
		[
				...
				'email_priority' => 1,
				'email_subject' => 'Business Membership Paymemt Confirmation',
	...
	public function stripePaymentUsersPost()
    {
        $stripe = getPaymentGateway('stripe');
        if (empty($stripe)) {
            setErrorMessage("Payment method not found!");
        ...
        if (!empty($paymentObject) && $paymentObject->client_secret == $clientSecret) {
            ...
            //add order
            $response = $this->executePaymentUsers($dataTransaction, $paymentSession->payment_type, langBaseUrl());
            if ($response->result == 1) {
                setSuccessMessage($response->message);
                echo json_encode([
                    'result' => 1,
	...
	public function paypalPaymentUsersPost()
    {
        $paypal = getPaymentGateway('paypal');
        if (empty($paypal)) {
            setErrorMessage("Payment method not found!");
        ...
        if ($paypalLib->getOrder($paymentId)) {
            ...
            $mdsPaymentType = inputPost('mds_payment_type');
            //add order
            $response = $this->executePaymentUsers($dataTransaction, $mdsPaymentType, langBaseUrl());
            if ($response->result == 1) {
                setSuccessMessage($response->message);
                echo json_encode([
                    'result' => 1,
	...
	public function executePaymentUsers($dataTransaction, $paymentType, $baseUrl)
    {
        //response object
        $response = new \stdClass();
        $response->result = 0;
        ...
        if (!$this->orderModel->isValidTransaction('sale', $dataTransaction['payment_id'], $dataTransaction['payment_method'])) {
            ...
            $response->message = 'Invalid transaction Id!';
            $response->result = 0;
            $response->redirectUrl = $baseUrl . getRoute('membershipPlansUsers', true) . getRoute('payment_users') . $params;
            return $response;
        }
		if ($paymentType == 'membership') {
            $planId = helperGetSession('modesy_selected_membership_plan_id');
            ...
            if (!empty($plan)) {
                //add user membership plan
                $this->membershipModel->addUserPlanSubs($dataTransaction, $plan, user()->id);
                //add transaction
                $this->membershipModel->addMembershipTransactionUsers($dataTransaction, $plan);
                //set response and redirect URLs
                $response->result = 1;
                $response->redirectUrl = $baseUrl . getRoute('membership_payment_completed_users') . '?method=gtw';
            } else {
                //could not added to the database
                $response->message = trans("msg_payment_database_error");
                $response->result = 0;
                $response->redirectUrl = $baseUrl . getRoute('membershipPlansUsers', true) . getRoute('payment_users') . '?payment_type=membership';
            }
        } 
