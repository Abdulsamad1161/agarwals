...
function onScanSuccess(qrCodeMessage) {
    swal({
        title: "Success!",
        text: "Successfully scanned the QR code. Remove the camera now, updating attendance.",
        type: "success",
        ...
        if (result) {
            ...
            // Get the target time zone dynamically from PHP
            const targetTimeZone = "<?php echo $timezone; ?>";
            const currentTimeInTargetZone = new Date().toLocaleTimeString("en-US", {
            ...
            $.ajax({
                ...
                data: setAjaxData(data), // Ensure setAjaxData is handling data formatting correctly
                dataType: 'json',
                success: function (response) {
                    if (response == 'updated') {
                        ...
                        swal({
                            title: "Error",
                            text: "Failed to scan the code, no data found.",
                            type: "warning",
                            confirmButtonColor: "#DD6B55",
                            confirmButtonText: "OK",
                        }).then(() => {
                ...
                error: function (xhr, textStatus, errorThrown) {
                    // Show error alert in case of AJAX failure
                    swal({
                        title: "Error",
