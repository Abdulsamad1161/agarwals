<div class="row" style="margin-bottom: 15px;">
    <div class="col-sm-12">
        <h3 style="font-size: 18px; font-weight: 600;"><?= trans('event_feedbacks'); ?></h3>
    </div>
</div>
<div class="box">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped data_table" role="grid" aria-describedby="example1_info">
                        ...
                        <tr role="row">
                            ...
                            <th><?= trans('name'); ?></th>
                            <th><?= trans('email'); ?></th>
                            <th><?= trans('event_details'); ?></th>
                            <th><?= trans('food_rating'); ?></th>
                            <th><?= trans('decor_rating'); ?></th>
                            <th><?= trans('cultural_rating'); ?></th>
                            <th><?= trans('registration_rating'); ?></th>
                            <th><?= trans('overal_rating'); ?></th>
                        ...
                        if (!empty($comments)):
                            foreach ($comments as $item):?>
                                <tr>
                                    ...
                                    <td><?= esc($item->event_name_date); ?></td>
                                    <td><?= esc($item->dining_experience); ?></td>
                                    <td><?= esc($item->decor_experience); ?></td>
                                    <td><?= esc($item->culturalProgram_experience); ?></td>
                                    <td><?= esc($item->registration_experience); ?></td>
                                        ...
                                        <form action="<?= base_url('AdminController/approvefeedbackPost'); ?>" method="post">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu options-dropdown">
                                                    ...
                                                    <?php endif; ?>
                                                    <li><a href="javascript:void(0)" onclick="deleteItem('AdminController/deletefeedback','<?= $item->id; ?>','<?= trans("confirm_feedback", true); ?>');"><i class="fa fa-trash option-icon"></i><?= trans('delete'); ?></a></li>
                                                </ul>
                                            </div>
                                        </form>
                                    </td>
