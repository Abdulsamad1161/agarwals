<?php if (!empty($paymentGateway) && $paymentGateway->name_key == 'paypal'): ?>
    <script src="https://www.paypal.com/sdk/js?client-id=<?= $paymentGateway->public_key; ?>&locale=en_CA&currency=<?= $currency; ?>"></script>
    <div class="row">
        <div class="col-12">
        ...
        paypal.Buttons({
            createOrder: function (data, actions) {
                return actions.order.create({
                    purchase_units: [{
						...
						description: 'Ticket Booking - Event : <?= helperGetSession('eventName'); ?> By <?= helperGetSession('mds_ses_username'); ?>'
                    }]
                });
            },
            onApprove: function (data, actions) {
                return actions.order.capture().then(function (details) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: MdsConfig.baseURL + '/paypal-payment-post-ticket', //paypal-payment-post
